...
export default function({width, init}) {
	let par = [], parCantidad = 20, distancia, mapeo, slideVel, slideCant;
	
	const setup = (p5,parent) => {
		p5.canvas = p5.createCanvas(width,width).parent(parent);
		slideVel = p5.createSlider(0,100,1).parent(parent);
		slideCant = p5.createSlider(50,300,50).parent(parent);
	};

	const draw = function(p5) {
		p5.background(200,0,0);
		if (par.length < parCantidad) par.push(new Dot(p5));
		...
		for (var i = par.length - 1; i >= 0; i--) {
			...
			for (var j = par.length - 1; j >= 0; j--) {
				...
				if (distancia < slideCant.value()){
					p5.stroke(255,255,255,p5.map(distancia,0,slideCant.value(),255,0));
	...
	const Dot = function(p5) {
		...
		this.show = function(p5) {
			// this.vel = p5.map(p5.mouseY>0?p5.mouseY:0,0,window.innerHeight,1,15);
			this.vel = slideVel.value() / 10;
			// Direccionamiento
